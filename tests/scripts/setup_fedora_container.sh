...
sleep 2
machinectl shell fedora /usr/bin/postgresql-setup --initdb
machinectl shell fedora /usr/bin/sed "s/#listen_addresses = 'localhost'/listen_addresses = '*'/" /var/lib/pgsql/data/postgresql.conf -i
machinectl shell fedora /usr/bin/bash -c 'echo "host all all 0.0.0.0/0 trust" > /var/lib/pgsql/data/pg_hba.conf'
machinectl shell fedora /usr/bin/systemctl enable postgresql --now


machinectl shell fedora /usr/sbin/ip addr add 172.16.0.1/30 dev host0
machinectl shell fedora /usr/sbin/ip link set up host0
