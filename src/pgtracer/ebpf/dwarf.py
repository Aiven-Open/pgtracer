...
def find_debuginfo(program_path: Path) -> Optional[ELFFile]:
    """
    ...
    """
    elf_file = ELFFile(program_path.open("rb"))
    ...
    buildid = extract_buildid(elf_file)
    if buildid:
...
class ProcessMetadata:
    ...
    def __init__(self, process: Process):
        ...
        elffile = find_debuginfo(self.program)
        if elffile is None:
        ...
        self.naive_index: Dict[str, Set[int]] = defaultdict(set)
        if gdb_index_section is not None:
            ...
            for cu in self.dwarf_info.iter_CUs():
                for die in cu.get_top_DIE().iter_children():
                    name = die_name(die)
                    if name is not None:
                        self.naive_index[name].add(die.offset)
    ...
    def _naive_die_search(self, tag: str, name: str) -> Generator[DIE, None, None]:
        ...
        """
        for offset in self.naive_index[name]:
            die = self.dwarf_info.get_DIE_from_refaddr(offset)
    ...
    def function_addresses(self, function_name: str) -> Generator[int, None, None]:
        ...
        """
        dies = self.search_symbol("DW_TAG_subprogram", function_name)
